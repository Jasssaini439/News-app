...
const Business_API_URL = `https://newsapi.org/v2/top-headlines?country=us&category=technology&apiKey=7074e877dfd44d1c84572a697eaa019d
`;
...
const Business = () => {
  ...
  useEffect(() => {
    ...
    const fetchBusinessNews = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${Business_API_URL}&page=${page}&pageSize=20`);
        setProgress(70);
  ...
  return (
    <div>
      <LoadingBar
        ...
        progress={progress}
      />
      <h1 className="font-semibold text-center m-20">Business News</h1>
      <InfiniteScroll
